...
1. cd .. 退出一层克隆远程仓库的gh-pages分支并保存为**book\_**_**build**文件夹:   _`git clone -b gh-pages https://github.com/luggege/book.git book_build`
2. 然后从本地编辑器自动生成的源目录中找到_book文件夹\(保存的html静态文件\),**复制**其中的静态文件和样式文件\(gitbook\)到   book\_build文件中_
3. 将静态文件**push**到远程仓库book的gh-pages分支上: 命令如下:
